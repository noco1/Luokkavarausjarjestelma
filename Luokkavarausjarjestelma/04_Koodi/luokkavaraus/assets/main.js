async function api(path, method='GET', body=null) 
{
    ...
    const res = await fetch('/luokkavaraus/luokkavaraus/api/' + path, opts);
    return res.json();
}
async function init() 
{
...
document.getElementById('loginBtn').addEventListener('click', async ()=>
{
    ...
    try {
        const data = await api('login.php', 'POST', {email, password});
        if (data.status === 'success') location.reload();
        else document.getElementById('loginMsg').textContent = data.message || 'Virhe';
    } catch (err) {
        document.getElementById('loginMsg').textContent = 'Virhe kirjautumisessa';
    }
});
